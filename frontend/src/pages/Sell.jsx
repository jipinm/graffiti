...
import { ArrowLeft, ArrowRight, Check, ShieldCheck, Phone as PhoneIcon, Sparkles } from "lucide-react";
...
const STEPS = ["brand", "model", "variant", "questionnaire", "otp", "price"];
...
export default function Sell() {
  ...
  const finalPrice = useMemo(() => {
    ...
    // Questionnaire impacts
    QUESTIONNAIRE_STEPS.forEach((s) => {
      s.questions.forEach((q) => {
        const ans = answers[q.id];
        if (q.invert) {
          // Issue → deduct if yes
          if (ans === true) price -= q.impact;
        } else {
          // Working feature → deduct if no
          if (ans === false) price -= q.impact;
        }
      });
    });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14" data-testid="sell-page">
        ...
        <div className="recell-fade-up" data-testid="sell-stage-variant">
          ...
          <div className="grid lg:grid-cols-12 gap-8 mt-6">
            ...
            <div className="lg:col-span-5">
              <div className="bg-[#1D1D1F] text-white rounded-3xl p-8" data-testid="price-estimator-card">
                <div className="text-xs uppercase tracking-wider font-semibold text-white/60">Estimated range</div>
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-4xl font-semibold recell-price-counter" style={{fontFamily:"Outfit"}}>
                    Get up to {formatINR(estimateRange[1])}
                  </span>
                </div>
      ...
      {stage === "questionnaire" && (
        <div className="recell-fade-up" data-testid="device-condition-form">
          ...
          <div className="mt-6 h-1.5 w-full bg-[#F5F5F7] rounded-full overflow-hidden">
            <div
              className="h-full bg-[#0066CC] transition-all duration-500"
              style={{ width: `${((qStep+1)/totalQs)*100}%` }}
            />
          </div>

          <div className="mt-8 bg-white border border-[#E5E5EA] rounded-3xl p-6 lg:p-10">
            <div className="space-y-4">
              {QUESTIONNAIRE_STEPS[qStep].questions.map((q) => {
                const isYesNo = !QUESTIONNAIRE_STEPS[qStep].selectAll;
                const current = answers[q.id];
                if (isYesNo) {
                  return (
                    <div key={q.id} data-testid={`question-${q.id}`} className="flex items-center justify-between gap-4 py-3 border-b border-[#F5F5F7] last:border-0">
                      <div className="text-sm sm:text-base">{q.label}</div>
                ...
                // Select-all multi-checkbox style
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-center justify-between gap-4 px-4 py-3 rounded-2xl border transition-colors ${
                      checked ? "border-[#0066CC] bg-[#0066CC]/5" : "border-[#E5E5EA] hover:border-[#1D1D1F]"
                    }`}
                  >
                    <span className="text-sm sm:text-base">{q.label}</span>
                    <span className={`h-5 w-5 rounded-md flex items-center justify-center border ${
                      checked ? "bg-[#0066CC] border-[#0066CC]" : "border-[#86868B]"
                    }`}>
            ...
            <div className="mt-8 flex items-center justify-between gap-3 flex-wrap">
              ...
              <button
                ...
                onClick={() => {
                  if (qStep < totalQs - 1) setQStep(qStep + 1);
                  else setStage("otp");
                }}
              ...
              {!otpSent ? (
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (otp === "123456") { setStage("price"); toast.success("Verified. Here's your exact price."); }
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7">
              ...
              <div className="mt-8 bg-[#1D1D1F] text-white rounded-3xl p-8 lg:p-10">
                ...
                <div className="mt-2 text-sm text-white/60">
                  Minimum Guarantee · Paid the day you walk in
                </div>
                <div className="mt-8 grid sm:grid-cols-2 gap-3">
                  <button
                    data-testid="sell-now-cta"
                    onClick={() => { toast.success("Listing created. Track it on your dashboard."); navigate("/dashboard"); }}
                ...
                <p className="mt-6 text-xs text-white/50 leading-relaxed">
                  Final amount is confirmed after a 24-hour lab inspection. If your phone resells above MG within 21 days, you earn a profit share.
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <SidePanel
                ...
                items={[
                  "Walk in to a ReCell store with a valid ID",
...
function StepBar({ stage }) {
  ...
  return (
    <div className="mb-8 hidden md:flex items-center gap-2" data-testid="sell-stepbar">
      {labels.map((l, i) => (
        <div key={l} className="flex items-center gap-2">
          <div className={`h-7 px-3 rounded-full text-xs font-medium inline-flex items-center justify-center ${
  ...
  return (
    <div className="bg-[#F5F5F7] rounded-3xl p-6">
      ...
      <ul className="mt-3 space-y-2">
        {items.map((it) => (
          <li key={it} className="flex items-start gap-2 text-sm text-[#1D1D1F]">
